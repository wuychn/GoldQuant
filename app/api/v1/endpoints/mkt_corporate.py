...
async def em_zh_a_hist_pre_min(
    ...
    return _ak("stock_zh_a_hist_pre_min_em", p, df)
...
async def em_gsrl_gsdt(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return _ak("stock_gsrl_gsdt_em", p, df)
...
async def em_zh_a_new() -> AkTableOut:
    p: dict = {}
    try:
        df = await run_in_threadpool(ak.stock_zh_a_new_em)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return _ak("stock_zh_a_new_em", p, df)
...
async def em_comment_scrd_focus(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return _ak("stock_comment_detail_scrd_focus_em", p, df)
...
async def em_comment_scrd_desire(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return _ak("stock_comment_detail_scrd_desire_em", p, df)
...
async def ths_xgsr() -> AkTableOut:
    try:
        df = await run_in_threadpool(ak.stock_xgsr_ths)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return _ak("stock_xgsr_ths", {}, df)
...
async def ths_zyjs(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return _ak("stock_zyjs_ths", p, df)
