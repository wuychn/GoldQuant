...
async def em_stock_changes(
    ...
    return Response(data=_ak("stock_changes_em", p, df))
...
async def em_board_change() -> Response:
    try:
        df = await run_in_threadpool(ak.stock_board_change_em)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_board_change_em", {}, df))
...
async def em_zt_pool(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_zt_pool_em", p, df))
...
async def em_zt_pool_previous(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_zt_pool_previous_em", p, df))
...
async def em_zt_pool_strong(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_zt_pool_strong_em", p, df))
...
async def em_zt_pool_sub_new(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_zt_pool_sub_new_em", p, df))
...
async def em_zt_pool_zbgc(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_zt_pool_zbgc_em", p, df))
...
async def em_zt_pool_dtgc(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_zt_pool_dtgc_em", p, df))
