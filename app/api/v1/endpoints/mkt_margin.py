...
async def em_margin_account_info() -> Response:
    try:
        df = await run_in_threadpool(ak.stock_margin_account_info)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_margin_account_info", {}, df))
...
async def em_margin_sse(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_margin_sse", p, df))
...
async def em_margin_detail_sse(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_margin_detail_sse", p, df))
...
async def em_margin_szse(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_margin_szse", p, df))
...
async def em_margin_detail_szse(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return Response(data=_ak("stock_margin_detail_szse", p, df))
