...
from quant.data.sources.ths.industry import fetch_ths_industry_names


class EmIndustryFetchTests(unittest.TestCase):
    @patch("quant.data.sources.eastmoney.industry.requests.get")
...
class ThsIndustryFetchTests(unittest.TestCase):
    @patch("quant.data.sources.ths.industry.requests.get")
    @patch("quant.data.sources.ths.industry.get_v", return_value="abc")
