...
def http_status_from_exception(exc: BaseException) -> int | None:
    ...
    采用鸭子类型：任何携带 ``http_status`` / ``status_code`` 属性的异常都可被提取，
    避免 common 反向依赖 quant/fastapi 的具体异常类。
    """
    ...
    # web 层异常（如 FastAPI HTTPException）携带 status_code；排除 502（网关错误，非远端状态）
    sc = getattr(exc, "status_code", None)
    if sc is not None and sc != 502:
        return int(sc)
