...
def main() -> None:
    ap = argparse.ArgumentParser(description="回测正确性检验：随机 alpha 基准与前视泄漏探测")
    add_home_argument(ap)
    ap.add_argument("--start", required=True, help="检验起始日 YYYY-MM-DD（含）")
    ap.add_argument("--end", required=True, help="检验结束日 YYYY-MM-DD（含）")
    ap.add_argument("--max-positions", type=int, default=10, help="最大持仓只数（默认 10）")
    ap.add_argument("--seed", type=int, default=42, help="随机 alpha 的 numpy 随机种子（默认 42）")
    args = ap.parse_args()
