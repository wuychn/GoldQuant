...
def main() -> None:
    ...
    dates = [d.strftime("%Y%m%d") for d in trading_days_between(args.start, args.end)]
    if not dates:
        print("无交易日")
        return
    ...
    broker = run_backtest(daily=daily, dates=dates, alpha_fn=alpha_fn, policy=policy, max_positions=args.max_positions)
