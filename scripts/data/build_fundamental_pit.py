...
def main() -> None:
    ap = argparse.ArgumentParser(description="批量构建 fundamental_pit 财务表（公告日 PIT）")
    add_home_argument(ap)
    ap.add_argument("--codes", default=None, help="逗号分隔股票代码；默认取 universe 全量")
    ap.add_argument("--limit", type=int, default=0, help="最多拉取只数（0 表示不限）")
    ap.add_argument("--batch-size", type=int, default=50, help="每批写入只数（保留兼容，实际按库内批量写）")
    ap.add_argument("--sleep", type=float, default=0.3, help="单只请求间隔秒数，用于限流（默认 0.3）")
    ap.add_argument("--retries", type=int, default=3, help="单只拉取失败时的重试次数（默认 3）")
    ap.add_argument("--resume", action="store_true", help="跳过 parquet 中已有代码，断点续跑")
    args = ap.parse_args()
