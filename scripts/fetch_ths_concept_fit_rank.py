...
def get_concept_fit_rank_list(symbol: str, *, timeout: float = 30.0) -> list[dict[str, Any]]:
    ...
    Returns:
        按排名升序的列表，每项为 {"rank": int, "concept": str}。
    """
    return [{"rank": row.rank, "concept": row.concept} for row in fetch_concept_fit_ranks(symbol, timeout=timeout)]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"股票 { _normalize_symbol(args.symbol) } 概念贴合度排名（共 {len(rows)} 个）:")
        for row in rows:
...
if __name__ == "__main__":
    r = get_concept_fit_rank_list('000636')
