...
【涨停板战法｜盘中买入】
条件自查：JSON「同花顺人气榜」含人气 **前 20 条**（轻量字段）；买卖判定仍以 **人气排名≤10** 为准；标的所属概念须对应「概念板块」之「涨幅榜」前十或「资金流入榜」前十；个股盘口请用「自选股」/「持仓股」或另行行情，勿假定人气榜条目含盘口。若标的仅在人气榜第11～20名，不按涨停战法盘中追击。盘口「最新」须高于「均价」（分时均线）方可买入。
输出：标的代码与名称，方向买入，仓位{x%}，委托价与时刻，止损与目标价。

【龙回头战法｜盘中买入】
条件自查：现价处于 5/10/20 日均线允许区间（见「5日线」「10日线」「20日线」或「技术指标」）；量能参考盘口「量比」；资金参考「个股资金流」当日记录的「主力净流入-净额」。
输出：委托价、时刻、仓位、止损。
