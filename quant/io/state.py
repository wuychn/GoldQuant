...
from quant.domain.models import PoolMember, PoolStage, SectorRow
from quant.store.paths import quant_home


def strategy_state_dir() -> Path:
    """策略状态目录（~/.quant/state/）。"""
    import shutil

    root = quant_home() / "state"
    legacy = root / "r2"
    root.mkdir(parents=True, exist_ok=True)
    if legacy.is_dir():
        for name in ("regime.json", "sector_snapshot.json", "tracking_pool.json", "combat_pool.json"):
            src = legacy / name
            dst = root / name
...
def save_regime(data: dict) -> None:
    _write_json(strategy_state_dir() / "regime.json", data)


def load_regime() -> dict:
    return _read_json(strategy_state_dir() / "regime.json") or {}
...
def save_sector_snapshot(rows: list[SectorRow]) -> None:
    ...
    _write_json(strategy_state_dir() / "sector_snapshot.json", payload)


def load_sector_snapshot() -> list[dict]:
    raw = _read_json(strategy_state_dir() / "sector_snapshot.json")
    ...
    for row in raw or []:
        ...
        out.append(
            PoolMember(
                ...
                alpha_score=float(row.get("alpha_score") or row.get("structure_score") or 0),
...
def save_tracking(members: list[PoolMember]) -> None:
    _write_json(strategy_state_dir() / "tracking_pool.json", _pool_to_json(members))


def save_combat(members: list[PoolMember]) -> None:
    _write_json(strategy_state_dir() / "combat_pool.json", _pool_to_json(members))


def load_tracking() -> list[PoolMember]:
    return _pool_from_json(_read_json(strategy_state_dir() / "tracking_pool.json") or [])


def load_combat() -> list[PoolMember]:
    return _pool_from_json(_read_json(strategy_state_dir() / "combat_pool.json") or [])
...
def _quote_is_stale(row: dict) -> bool:
    """历史 K 最新一根不是今天 → 盘口涨跌幅不可信。"""
    ...
    return bool(ds and ds != cn_date_str())
...
def inject_combat_watchlist(
    ...
    *,
    ...
    out = dict(payload)
    live_quotes = _quote_index(payload)
    ...
    for m in members:
        ...
        live = live_quotes.get(m.code) or {}
        if live:
            base = {**snap, **live}
            if _quote_is_stale(base):
                base.pop("盘口", None)
                base["quote_stale"] = True
        else:
            # 盘中 payload 无实时行情时，不用 snapshot 里陈旧盘口（避免 +19.99% 等误导）
            base = dict(snap)
            base.pop("盘口", None)
            base["quote_stale"] = True
        base.update(
