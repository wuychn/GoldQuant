"""市场状态推断（全市场绝对状态，与截面 alpha 正交）。

从 ``quant.scoring.context`` 迁出：r1 评分体系退役后，``infer_regime`` 仍被
narrative（推送叙事"赚钱效应强/一般/差"）使用。这里是纯 payload 读取，无 scoring 依赖。
"""
...
def infer_regime(payload: dict) -> str:
    """由大盘指数/赚钱效应/涨停统计推断市场档位（强势/震荡/弱势）。"""
