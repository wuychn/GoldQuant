...
async def _request_json(
    ...
    client_kw = client_kw or {"timeout": HTTP_TIMEOUT}
    try:
...
async def call_ths_api(url: str) -> Any:
    ...
    client_kw: dict[str, Any] = {"timeout": HTTP_TIMEOUT}
...
async def stock_skyrocket(settings):
    """同花顺人气飙升榜。"""
    result = []
    for stock in await _fetch_hot_list("skyrocket"):
        try:
            if _is_a_share_code(stock["code"]):
...
async def ggzjl(symbol):
    """个股资金流（TTL 缓存 + 退避重试 + 限流）。"""
    from quant.data.sources.ths import fetch_stock_funds

    return await fetch_stock_funds(symbol)
