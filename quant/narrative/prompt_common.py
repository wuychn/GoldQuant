...
def data_semantics_note(mode: str) -> str:
    ...
    if mode in _INTRADAY_SEMANTICS_MODES:
        ...
        notes.append(
            ...
            "勿纳入本轮新入选。"
            "三、持仓股表现：依据写作参考「持仓股表现」与 JSON「持仓股」，写持股涨跌与盘面强弱；"
            "不写买卖动作。"
            "四、操作复盘：依据「今日操作」与 JSON「持仓股」，复盘当日买卖、持股、做T 等；"
            "无成交须写「今日无买卖操作」，勿虚构操作。"
            "五、盈亏总结：严格依据「当日盈亏参考」；无成交且浮动有限时写基本持平，"
            "禁止写「大幅盈利/亏损」等与数据不符的表述。\n"
        )
    if mode == "post_market_lunch":
        notes.append(
            "【午间专节】三、持仓股表现：依据写作参考「持仓股表现」，写持股上午表现；"
            "四、下午策略：只写午后盘面节奏与关注方向，禁止写「操作」「买卖」「加仓」「减仓」"
            "「无买卖」等交易动作。\n"
        )
