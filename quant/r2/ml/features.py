...
from quant.r2.io.payload import market_snapshot, popularity_rows, stock_rows
from quant.r2.sector.engine import build_sector_rows as sector_rows_from_payload
...
def build_sector_feature_rows(date: str, payload: dict, *, source: str) -> list[dict]:
    rows = sector_rows_from_payload(payload)
    out: list[dict] = []
    for r in rows:
        out.append(
            {
