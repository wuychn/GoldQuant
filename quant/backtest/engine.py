...
from quant.config import load_r2_config, override_r2_config
from quant.backtest.metrics import compute_metrics
from quant.io.quotes import build_stock_by_code
from quant.io.state import inject_combat_watchlist, reset_pools
...
def _patch_broker(broker: SimBroker, pending_box: list[dict], clock: list[datetime]) -> Iterator[None]:
    ...
    with (
        ...
        patch("quant.signals.pipeline.codes_sold_today", _sold),
        patch("quant.signals.pipeline.holding_codes_bought_today", _bought),
...
def run_backtest(
    *,
    ...
    sector_hist: list[dict] = []

    with override_r2_config(cfg):
        for d in dates:
