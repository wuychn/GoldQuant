...
def prompt_pre_market_main() -> str:
    return (
        ...
        + "有数据则逐只列表输出：序号、名称(代码) [战法]、加入原因摘要）\n\n"
        + "三、持仓股\n"
        + "（仅 JSON「持仓股」。为空则写「当前无持仓」。"
        + "有数据则逐只列表输出：序号、名称(代码)、买入价、买入原因摘要）\n\n"
        + "六、市场判断与当日执行纲要\n\n"
...
def prompt_pre_market_zt() -> str:
    return (
        ...
        + "【输出格式】仅输出「四、涨停板战法」，逐只自选股用编号列表输出集合竞价分析：\n"
        + "1、{名称}({代码}) — {集合竞价判定}（高开幅度、竞价量能、封单情况）\n"
        + "2、{名称}({代码}) — {判定}\n"
        + "最后汇总盘中操作计划。\n"
        + LLM_OUTPUT_FORMAT
    )
...
def prompt_pre_market_lht() -> str:
    return (
        ...
        + "需在复盘时从人气榜筛选加入），不可省略本小节。\n"
        + "【输出格式】仅输出「五、龙回头战法」，逐只自选股用编号列表输出定价分析：\n"
        + "1、{名称}({代码}) — {定价与资金口径判定}（均线位置、MACD、资金流向）\n"
        + "2、{名称}({代码}) — {判定}\n"
        + "最后汇总盘中操作计划。\n"
        + LLM_OUTPUT_FORMAT
    )
...
def prompt_during_buy_zt() -> str:
    return (
        ...
        + "【输出格式】以「【涨停板战法｜盘中买入】」开头，逐只自选股用编号列表输出分析：\n"
        + "1、{名称}({代码}) — {买入信号判定结论}（盘口要点、概念共振情况）\n"
        + "2、{名称}({代码}) — {结论}\n"
        + "最后一行汇总：委托建议或无操作。\n"
        + LLM_OUTPUT_FORMAT
    )
...
def prompt_during_buy_lht() -> str:
    return (
        ...
        + "原因：{说明为何为空，如复盘未筛出合格标的、数据不完整等}。不可省略本段。\n"
        + "【输出格式】以「【龙回头战法｜盘中买入】」开头，逐只自选股用编号列表输出分析：\n"
        + "1、{名称}({代码}) — {买入信号判定结论}（均线位置、量能、MACD、资金流向）\n"
        + "2、{名称}({代码}) — {结论}\n"
        + "最后一行汇总：委托建议或无操作。\n"
        + LLM_OUTPUT_FORMAT
    )
...
def prompt_during_hold_zt() -> str:
    return (
        ...
        + "不可省略本段。\n"
        + "【输出格式】以「【涨停板战法｜持仓与卖出】」开头，逐只持仓股用编号列表输出监控：\n"
        + "1、{名称}({代码}) 买入价{x}→现价{y} {盈亏%} — {持仓状态/卖出信号判定}\n"
        + "2、{名称}({代码}) …\n"
        + "最后一行汇总：卖出委托建议或继续持有。\n"
        + LLM_OUTPUT_FORMAT
    )
...
def prompt_during_hold_lht() -> str:
    return (
        ...
        + "不可省略本段。\n"
        + "【输出格式】以「【龙回头战法｜持仓与卖出】」开头，逐只持仓股用编号列表输出监控：\n"
        + "1、{名称}({代码}) 买入价{x}→现价{y} {盈亏%} — {持仓状态/卖出信号判定}（均线、资金流）\n"
        + "2、{名称}({代码}) …\n"
        + "最后一行汇总：卖出委托建议或继续持有。\n"
        + LLM_OUTPUT_FORMAT
    )
